...
class Game {
    ...
    constructor(anypixel) {
        ...
        this.player = new Player("#FFF", 15, 3);
        this.background = "#000";
        this.barsPerRow = 7;
        this.barRows = 5;
        this.bars = [];
        this.barColors = ["#4ffdff", "#ff392b", "#f81cff", "#4cff45"];
    ...
    collide() {
        if(this.player.left <= 0 || this.player.right >= this.config.width) this.player.vel.x = 0;
        ...
        if(this.ball.bottom >= this.config.height) {
            ...
            loop1: for (let i = 0; i < this.barRows; i++) {
                for (let j = 0; j < this.barsPerRow; j++) {
        ...
        // if(this.ball.left <= this.player.right && this.ball.right >= this.player.left && this.ball.top <= this.player.bottom && this.ball.bottom >= this.player.top) {
            ...
            this.ball.vel.x += this.ball.vel.x * (Math.random() * (.5));
